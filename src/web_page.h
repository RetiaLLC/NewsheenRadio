...
let gZoom=1,gPinch=0,gCountry=null;
...
function gSetZoom(z){
 ...
 if(gZoom>=1.6) globeViewportSoon();
 else if(was>=1.6&&gCountry) globeCountry();   // zoomed back out: restore the spread
 if(!gRaf) globeDraw();
}
function globeDraw(){
...
function globeInit(){
 ...
 if(!c._wired){
  ...
  const up=e=>{
   ...
   const spun=gDrag&&moved>=7;
   gDrag=false;c.style.cursor='grab';gLastMove=Date.now();
   // Panning while zoomed moves the window over the catalogue, so reload the
   // stations that are now on screen.
   if(spun&&gZoom>=1.6) globeViewportSoon();
  };
...
function loadCountries(){
 ...
 fetch('/api/countries').then(r=>r.json()).then(d=>{
  ...
  for(const c of d){
   ...
   o.value=c.off+':'+c.n+':'+(c.len||0); o.textContent=c.name+' ('+c.n+')';
   sel.appendChild(o);
  }
 }).catch(()=>{});
}
...
function globeCountry(){
 ...
 const [off,n,len]=v.split(':');
 gCountry={off:+off,n:+n,len:+len||0};
 const h=document.getElementById('ghint'); h.textContent='loading…';
 // Sample ACROSS the country rather than taking the first N rows. The
 // catalogue is ordered alphabetically by place, so a head-of-range request
 // returned only the "A" towns — the United States showed Atlanta and
 // Albuquerque and none of San Francisco or Los Angeles.
 const cap=Math.min(300,+n);
 const q=gCountry.len
   ? '/api/catalogue?off='+off+'&len='+gCountry.len+'&sample='+cap
   : '/api/catalogue?off='+off+'&n='+cap;
 fetch(q).then(r=>r.json()).then(d=>{
  ...
  h.textContent=gStations.length+' of '+n+' stations — zoom in to load the rest';
  if(!gRaf) globeDraw();
 }).catch(()=>{h.textContent='could not load that country';});
}
// Visible angular radius of an orthographic globe drawn at radius W*0.415*gZoom:
// a point at angle t from centre projects to sin(t), and the canvas half-width
// is 1/(0.83*gZoom) sphere-radii, so everything with sin(t) below that is on
// screen. gZoom 1 shows a hemisphere; gZoom 6 shows about 12 degrees.
function gViewRadius(){
 return Math.asin(Math.min(1,1.2048/gZoom))*180/Math.PI;
}
// Pull the stations actually on screen. Only worth doing once zoomed past the
// point where a country-wide sample is thinner than the view.
function globeViewport(){
 if(gZoom<1.6||!gStations) return;
 const r=gViewRadius();
 const latMin=Math.max(-90,gTilt-r), latMax=Math.min(90,gTilt+r);
 const cosT=Math.max(0.25,Math.cos(gTilt*Math.PI/180));
 const pad=Math.min(180,r/cosT);
 let lonMin=gRot-pad, lonMax=gRot+pad;
 const wrap=v=>{while(v>180)v-=360;while(v<-180)v+=360;return v;};
 lonMin=wrap(lonMin); lonMax=wrap(lonMax);
 const bbox=[latMin.toFixed(2),latMax.toFixed(2),lonMin.toFixed(2),lonMax.toFixed(2)].join(',');
 let q='/api/catalogue?bbox='+bbox+'&n=600';
 if(gCountry&&gCountry.len) q+='&off='+gCountry.off+'&len='+gCountry.len;
 const h=document.getElementById('ghint');
 // Scanning a big country's rows off the filesystem takes a few seconds, so
 // say so rather than leaving the old count sitting there looking stale.
 if(h) h.textContent='loading stations in view…';
 fetch(q).then(r=>r.json()).then(d=>{
  const got=d.filter(s=>s.lat!=null&&s.lon!=null);
  if(got.length){
   gStations=got;
   h.textContent=got.length+' stations in view'+(got.length>=600?' (capped — zoom in further)':'');
   if(!gRaf) globeDraw();
  } else if(h){
   h.textContent='no stations in this view';
  }
 }).catch(()=>{if(h) h.textContent='could not load this view';});
}
let gVpTimer=0;
function globeViewportSoon(){
 clearTimeout(gVpTimer);
 gVpTimer=setTimeout(globeViewport,450);   // debounce: zooming is continuous
}
